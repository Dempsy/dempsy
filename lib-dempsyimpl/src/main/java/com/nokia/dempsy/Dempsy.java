...
/**
 * This class is the master orchestrator of the setup of a VM running the
 * application. It is intended to have the ApplicationDefinitions autowired in
 * so it picks up each one.
 */
public class Dempsy {
   ...
   public class Application {
      /**
       * Currently a "Dempsy.Application.Cluster" has no utility since it only has
       * a single Node. This may change
       */
      public class Cluster {
         /**
          * A Node is essentially an {@link MpContainer} with all of the attending
          * infrastructure. Currently a Node is instantiated within the Dempsy
          * orchestrator as a one to one with the {@link Cluster}.
          */
         public class Node implements MpClusterWatcher<ClusterInformation, SlotInformation> {
            ...
            private void start() throws DempsyException {
               try {
                  MpClusterSession<ClusterInformation, SlotInformation> clusterSession =
                        clusterSessionFactory.createSession();
                  ...
                  // there is only a reciever if we have an Mp (that is, we aren't an
                  // adaptor) and start accepting messages
                  if (messageProcessorPrototype != null && acceptedMessageClasses != null
                        && acceptedMessageClasses.size() > 0) {
                  ...
                  if (statsFactory != null) {
                     statsCollector =
                           statsFactory.createStatsCollector(currentClusterId, receiver != null ? receiver.getDestination()
                                 : null);
                  ...
                  // there is only an inbound strategy if we have an Mp (that is, we
                  // aren't an adaptor) and
                  // we actually accept messages
                  if (messageProcessorPrototype != null && acceptedMessageClasses != null
                        && acceptedMessageClasses.size() > 0)
                     strategyInbound = strategy.createInbound();

                  MpCluster<ClusterInformation, SlotInformation> currentClusterHandle =
                        clusterSession.getCluster(currentClusterId);

                  // this can fail because of down cluster manager server ... but it
                  ...
                  try {
                     ...
                     logger
                     .warn(
                           "Strategy failed to initialize. Continuing anyway. The cluster manager issue will be resolved automatically.",
                           e);
                  }
            ...
            public void process(MpCluster<ClusterInformation, SlotInformation> cluster) {
               ...
               // TODO: fix these catches... .need to take note of a failure for a
               // later retry
            ...
            public void stop() {
               if (receiver != null) {
                  try {
                     ...
                     logger.error("Error stoping the reciever " + SafeString.objectDescription(receiver) + " for "
                           + SafeString.valueOf(clusterDefinition) + " due to the following exception:", th);
                  }
               }
               ...
               if (container != null)
                  try {
                     ...
                     logger.error("Problem shutting down node for " + SafeString.valueOf(clusterDefinition), th);
                  }
            ...
            /**
             * Run any methods annotated PreInitilze on the MessageProcessor
             * prototype
             * 
             * @param prototype
            ...
            private void preInitializePrototype(Object prototype) {
               // TODO, should this return a boolean and stop the cluster from
               // starting?
               ...
               for (Method method : prototype.getClass().getMethods()) {
                  if (method.isAnnotationPresent(com.nokia.dempsy.annotations.Start.class)) {
                     try {
                        ...
                        break; // Only allow one such method, which is checked during
                        // validation
                     } catch (Exception e) {
      ...
      public class ClusterStart implements Runnable {
         ...
         private ClusterStart(Cluster cluster) {
      ...
      public boolean start() throws DempsyException {
         ...
         for (Cluster cluster : appClusters) {
            ...
            // {
            // Thread t = new Thread(new ClusterStart(cluster),"Starting cluster:" +
   ...
   public synchronized void start() throws DempsyException {
      ...
      if (defaultStatsCollectorFactory == null)
         throw new DempsyException(
               "Cannot start this application because there's no default stats collector factory defined.");
   ...
   // public void setMultithreadedStart(boolean multiThreadedStart) {
   // this.multiThreadedStart = multiThreadedStart; }
   ...
   @Inject
   public void setDefaultRoutingStrategy(RoutingStrategy defaultRoutingStrategy) {
      this.defaultRoutingStrategy = defaultRoutingStrategy;
   }

   @Inject
   public void setDefaultSerializer(Serializer<Object> defaultSerializer) {
      this.defaultSerializer = defaultSerializer;
   }

   @Inject
   public void setDefaultStatsCollectorFactory(StatsCollectorFactory defaultfactory) {
   ...
   /**
    * Wait for Dempsy to be stopped. This is useful in a 'main' that needs to
    * wait for an external shutdown to complete.
    * 
