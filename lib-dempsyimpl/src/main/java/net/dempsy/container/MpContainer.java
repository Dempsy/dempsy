...
public class MpContainer implements Listener, OutputInvoker, RoutingStrategy.Inbound.KeyspaceResponsibilityChangeListener {
    ...
    private final AnnotatedMethodInvoker keyMethods = new AnnotatedMethodInvoker(MessageKey.class);
    ...
    public void setPrototype(final Object prototype) throws ContainerException {
     ...
     */
    private void preInitializePrototype(final Object prototype) {
        for (final Method method : prototype.getClass().getMethods()) {
            if (method.isAnnotationPresent(net.dempsy.annotations.Start.class)) {
    ...
    public void setExecutor(final DempsyExecutor executor) {
    ...
    /**
     * Set the serializer. This can be injected to change the serialization scheme, but is optional. The default serializer uses Java Serialization.
     * 
     * @param serializer
     */
     ...
     */
    @Override
    public boolean onMessage(final byte[] data, final boolean fastfail) {
        Object message = null;
        try {
            message = serializer.deserialize(data, Object.class);
    ...
    /**
     * This is a helper class with unique synchronizataion behavior. It can be used to start a worker in another thread, allow the initiating thread to verify that's it's been started, and allow the early
     * termination of that worker thread.
     */
    private class RunningEventSwitch {
        ...
        /**
         * This allows the main thread to wait until the worker is started in order to continue. This method only works once. It resets the flag so a second call will block until another thread calls to
         * workerInitiateRun().
         */
    ...
    private void runExpandKeyspace(final RoutingStrategy.Inbound strategyInbound) {
        ...
        try {
            ...
            final Iterable<?> iterable = keySource.getAllPossibleKeys();
            ...
            if (keyspaceChangeSwitch.wasPreempted()) // this run is being preempted
            {
                for (final Future<Object> f : futures)
                    try {
                        ...
                        logger.warn("Error trying to cancel an attempt to pre-instantiate a Mp", th);
                    }
            }
        }
    }
    ...
    public class KeyspaceChanger implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (shrink) {
                    ...
                    doEvict(new EvictCheck() {
                        ...
                        public boolean shouldEvict(final Object key, final InstanceWrapper wrapper) {
                            return !strategyInbound.doesMessageKeyBelongToNode(key);
                        }
                ...
                keyspaceChangeSwitch.workerStopping();
            }
        }
    }
    ...
    protected boolean dispatch(final Object message, final boolean block) throws ContainerException {
        if (message == null)
     ...
     */
    protected InstanceWrapper getInstanceForDispatch(final Object message) throws ContainerException {
        if (message == null)
