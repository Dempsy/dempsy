...
import net.dempsy.container.MpContainer.InstanceWrapper;
...
public class TestInstanceManager {
    ...
    public static class CombinedMP
        ...
        @MessageHandler
        public String handle(final MessageOne message) {
        ...
        @MessageHandler
        public String handle(final MessageTwo message) {
            if (firstMessageTime < 0)
                firstMessageTime = System.nanoTime();
            messages.add(message);
            return "MessageTwo";
    ...
    public MpContainer setupContainer(final Object prototype) throws ContainerException {
        final DummyDispatcher dispatcher = new DummyDispatcher();
        final StatsCollector stats = new StatsCollectorCoda(new ClusterId("test", "test"), new StatsCollectorFactoryCoda().getNamingStrategy());
    ...
    public void tearDown() {
        ...
        final StatsCollector stats = manager.getStatsCollector();
        if (stats != null) {
    ...
    public void testSingleInstanceOneMessage() throws Throwable {
        ...
        final MpContainer manager = setupContainer(prototype);

        assertEquals("starts with no instances", 0, manager.getProcessorCount());

        final MessageOne message = new MessageOne(123);
    ...
    public void testSingleInstanceTwoMessagesSameClassSeparateExecution()
        ...
        final MessageOne message1 = new MessageOne(123);
        ...
        final MessageOne message2 = new MessageOne(123);
    ...
    public void testSingleInstanceTwoMessagesDifferentClassSeparateExecution()
        ...
        final MessageTwo message2 = new MessageTwo(123);
        assertSame("same wrapper returned for second message",
                wrapper, manager.getInstanceForDispatch(message2));
        manager.dispatch(message2, false);
        assertEquals("no other instance was created", 1, manager.getProcessorCount());
    ...
    public void testMultipleInstanceCreation() throws Exception {
        ...
        final MessageOne message2 = new MessageOne(456);
        final InstanceWrapper wrapper2 = manager.getInstanceForDispatch(message2);
        manager.dispatch(message2, false);
    ...
    public void testOutput() throws Exception {
        ...
        final MpContainer instanceManager = setupContainer(prototype);
        final DummyDispatcher dispatcher = ((DummyDispatcher) instanceManager.getDispatcher());

        // we need to dispatch messages to create MP instances
        final MessageOne message1 = new MessageOne(1);
        final InstanceWrapper wrapper1 = instanceManager.getInstanceForDispatch(message1);
        instanceManager.dispatch(message1, true);
        final MessageOne message2 = new MessageOne(2);
        final InstanceWrapper wrapper2 = instanceManager.getInstanceForDispatch(message2);
    ...
    public void testOutputShortCircuitsIfWrongClass() throws Exception {
        ...
        assertEquals("number of processed messages should include outputs.", 4,
                ((MetricGetters) instanceManager.getStatsCollector()).getProcessedMessageCount());
    }
    ...
    public void testFailureUnsupportedMessage() throws Exception {
        final MpContainer dispatcher = setupContainer(new UnsuportedMessageTestMP());
